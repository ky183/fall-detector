...
## 当前状态（v0.6 · 2026-09-22，终审冲刺版）

- ✅ 已闭环（真机实测）：采集 → ESP-NOW(50Hz, ch6) → 三阶段触发 + 随机森林判决 → 报警/取消 → 按需 WiFi + WxPusher 推送
- ✅ v0.6 功能包（详见 `docs/handover_2026-09-22_v0.6_ui_push_fix.md`）：
  - **腕端 OLED 产品化**：报警弹窗（大字倒计时）/已推送/已取消/正常页四级；调试布局由 `DEBUG_DISPLAY` 切换
  - **报警状态回显腰→腕**：协议预留的 `PKT_ACK` 正式启用，报警全程腕端屏上有画面
  - **步数统计（腕端纯本地）**：SVM 峰值检测 + 连续 4 步确认防误报，RAM 计数
  - **推送文案规范化** + **WiFi 重试卡死修复**（`cannot set config` 根因）+ 失败自动扫描诊断
- ⚠️ **两端固件须配套烧录 v0.6**（协议变更，混搭不崩但弹窗不出现）
- ⚠️ 演示硬性项：腰端供电 ≥6.65V（Micro USB 仅供烧录，蜂鸣器会声弱）；**上电前重启手机热点**（热点假死坑，见交接文档 §3.1）
- 待办：蜂鸣器供电诊断确认 → 步数按终审佩戴方式复调 → 重录演示视频 → 合并 main 打 `v0.6` tag

## 系统架构（双板 ESP-NOW）

| 端 | 硬件 | 职责 |
|----|------|------|
| 手腕 | XIAO ESP32S3（自制 PCB）+ MPU6050#1 + OLED + 按钮 + 电池 | 采集姿态 + 本地步数统计 + 报警回显显示 → ESP-NOW 发送 |
| 腰部 | ESP32-S3 + MPU6050#2 + 有源蜂鸣器 + EC11 编码器（取消） | 融合判断 + 两段式报警 + WiFi 推送 + 报警状态回传腕端 |
